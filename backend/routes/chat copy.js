...
router.post('/', async (req, res) => {
  ...
  if (!apiKey || apiKey.trim() === '' || apiKey.startsWith('your_')) {
    console.log('[Chat API] No OpenAI key. Running local chatbot fallback.');
...
- Keep your answers developer-friendly, clear, and structured. Use Markdown formatting.`;
  ...
  try {
    ...
    console.error('[Chat API] OpenAI Error:', error.message);
    const fallbackReply = generateLocalChatResponse(message, analysis, url, useCase);
    return res.json({ 
      reply: fallbackReply, 
      warning: 'OpenAI API call failed. Showing automated keyword response.' 
    });
  }
});
